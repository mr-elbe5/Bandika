...
CREATE OR REPLACE FUNCTION ADDPAGE (id INTEGER,parent_id INTEGER,name VARCHAR,display_name VARCHAR,
                                    description VARCHAR,user_id INTEGER, layout INTEGER)
    RETURNS VOID AS
$$
BEGIN
    ...
    INSERT INTO t_page (id, keywords, master)
    VALUES (id, '', 'defaultMaster');
    INSERT INTO t_section_page (id, layout)
    VALUES (id,layout);
END
$$
    LANGUAGE plpgsql;
...
CREATE TABLE IF NOT EXISTS t_section_page
(
    id          INTEGER      NOT NULL,
    layout      VARCHAR(255) NULL,
    CONSTRAINT t_section_page_pk PRIMARY KEY (id),
    CONSTRAINT t_section_page_fk1 FOREIGN KEY (id) REFERENCES t_page (id) ON DELETE CASCADE
);

CREATE SEQUENCE s_section_part_id START 1000;

CREATE TABLE IF NOT EXISTS t_section_part
(
    id            INTEGER      NOT NULL,
    type          VARCHAR(30)  NOT NULL DEFAULT 'SectionPartData',
    ...
    creation_date TIMESTAMP    NOT NULL DEFAULT now(),
    change_date   TIMESTAMP    NOT NULL DEFAULT now(),
    CONSTRAINT t_section_part_pk PRIMARY KEY (id),
    CONSTRAINT t_section_part_fk1 FOREIGN KEY (page_id) REFERENCES t_section_page (id) ON DELETE CASCADE
);

CREATE TABLE IF NOT EXISTS t_field_section_part
(
    id            INTEGER      NOT NULL,
    layout        VARCHAR(255) NOT NULL,
    CONSTRAINT t_field_section_part_pk PRIMARY KEY (id),
    CONSTRAINT t_field_section_part_fk1 FOREIGN KEY (id) REFERENCES t_section_part (id) ON DELETE CASCADE
);
...
(
    ...
    CONSTRAINT t_part_field_fk1 FOREIGN KEY (part_id) REFERENCES t_section_part (id) ON DELETE CASCADE
);

CREATE OR REPLACE FUNCTION ADDSECTIONPAGE (id INTEGER,parent_id INTEGER,name VARCHAR,display_name VARCHAR,
                                           description VARCHAR,user_id INTEGER, layout VARCHAR)
    RETURNS VOID AS
$$
BEGIN
    INSERT INTO t_content (id,type,parent_id,ranking,name,display_name,description,creator_id,changer_id,access_type,nav_type)
    VALUES (id,'SectionPageData',parent_id,0,name,display_name,description,user_id,user_id,'OPEN','HEADER');
    INSERT INTO t_page (id, keywords, master)
    VALUES (id, '', 'defaultMaster');
    INSERT INTO t_section_page (id, layout)
    VALUES (id,layout);
