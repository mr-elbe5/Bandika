...
public class TeamFileBean extends PersistenceBean {
    ...
    protected void writeFileData(Connection con, TeamFileData data) throws SQLException {
        PreparedStatement pst = null;
        try {
            String sql = data.isNew() ? "insert into t_teamfile (change_date,teampart_id,owner_id,owner_name,checkout_id,checkout_name,id) values (?,?,?,?,?,?,?)"
                    : "update t_teamfile set change_date=?,teampart_id=?,owner_id=?,owner_name=?,checkout_id=?,checkout_name=? where id=?";
    ...
    public List<TeamFileData> getFileList(int teampartId, int userId) {
    ...
    public List<TeamFileData> getFileHistory(int fileId) {
        List<TeamFileData> list = new ArrayList<>();
        Connection con = null;
        PreparedStatement pst = null;
        try {
    ...
    public void readFileList(PreparedStatement pst, List<TeamFileData> list) throws SQLException {
        int i;
        ResultSet rs = pst.executeQuery();
        while (rs.next()) {
            i = 1;
