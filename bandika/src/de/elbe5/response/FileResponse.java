/*
 ...
 This file uses ideas and includes parts of net/balusc/webapp/FileServlet.java of BalusC, Copyright (C) 2009 BalusC.
 */
...
public class FileResponse implements IResponse {
    ...
    private static final int DEFAULT_BUFFER_SIZE = 0x4000; // ..bytes = 16KB.
    ...
    public FileResponse(FileData data, boolean forceDownload) {
        this.data = data;
    ...
    {
        Log.warn("use of deprecated FileResponse");
        HttpServletRequest request = rdata .getRequest();
        String fileName = data. getTempFileName();
        long length = data.getFileSize();
        long lastModified = DateUtil.asMillis(data.getChangeDate());
        String eTag = fileName + "_" + length + "_" + lastModified;

        File file = new File(ApplicationPath.getAppFilePath() + "/" + fileName);
        if (!file.exists()) {
        ...
        try {
            ...
            Log.error("could not process file response", e);
