...
public class UserBean extends Bean {
  ...
  public UserData loginUser(String login, String pwd) {
    ...
    try {
      con = getConnection();
      pst = con.prepareStatement("select id,change_date,pwd,first_name,last_name,email,failed_login_count from t_user where login=? and approved=true and locked=false and deleted=false");
  ...
  protected void writeUser(Connection con, UserData data) throws SQLException {
    PreparedStatement pst = null;
    try {
      pst = con.prepareStatement(data.isBeingCreated() ? "insert into t_user (change_date,first_name,last_name,email,login,pwd,approval_code,approved,failed_login_count,locked,deleted,id) values(?,?,?,?,?,?,?,?,?,?,?,?)"
        : data.getPassword().length() == 0 ? "update t_user set change_date=?,first_name=?,last_name=?,email=?,login=?,approval_code=?,approved=?,failed_login_count=?,locked=?,deleted=? where id=?"
        : "update t_user set change_date=?,first_name=?,last_name=?,email=?,login=?,pwd=?,approval_code=?,approved=?,failed_login_count=?,locked=?,deleted=? where id=?");
      int i = 1;
  ...
  protected void writeUserProfile(Connection con, UserData data) throws SQLException {
    PreparedStatement pst = null;
    try {
      pst = con.prepareStatement(data.getPassword().length() == 0 ? "update t_user set change_date=?,first_name=?,last_name=?,email=? where id=?"
        : "update t_user set change_date=?,first_name=?,last_name=?,email=?,pwd=? where id=?");
      int i = 1;
      pst.setTimestamp(i++, data.getSqlChangeDate());
      pst.setString(i++, data.getFirstName());
      pst.setString(i++, data.getLastName());
  ...
  protected void writeGroup(Connection con, GroupData data) throws SQLException {
    PreparedStatement pst = null;
    try {
      pst = con.prepareStatement(data.isBeingCreated() ? "insert into t_group (change_date,name,id) values(?,?,?)"
        : "update t_group set change_date=?,name=? where id=?");
      int i = 1;
      pst.setTimestamp(i++, data.getSqlChangeDate());
