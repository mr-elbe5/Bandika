...
public class ControlTask implements TimerTask {
  ...
  public boolean execute(Date executionTime, Date checkTime) {
    Logger.info(null, String.format("control task executing task of %s at %s",
      Configuration.getDateTimeFormat().format(executionTime),
      Configuration.getDateTimeFormat().format(TimerBean.getInstance().getServerTime())));
    return true;
  }
