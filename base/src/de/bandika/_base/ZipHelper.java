...
public class ZipHelper {
  ...
  public static void ensureDirectory(String path) throws IOException {
    Logger.info(null, "ensure directory: " + path);
  ...
  public static void writeFile(ZipInputStream zin, String path, boolean replace) throws IOException {
    ...
    if (f.exists()) {
      if (replace) {
        f.delete();
        Logger.info(null, "replacing file: " + path);
      } else {
        Logger.info(null, "file exists already: " + path);
      }
    } else {
      Logger.info(null, "creating file: " + path);
    }
