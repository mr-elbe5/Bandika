...
public class Logger {
  ...
  protected void printLog(Class<?> cls, String message, Throwable t,
                          RequestData rdata) {
    if (!isValidThrowable(cls, t))
      return;
    StringBuffer buffer = new StringBuffer();
  ...
  protected boolean isValidThrowable(Class<?> cls, Throwable t) {
    if (t != null && t instanceof SocketException) {
      printLog(WARN, cls,
        "SocketException after response: " + t.getMessage());
      return false;
    }
    return true;
  }

  public static void log(Class<?> cls, String message, Throwable t,
                         RequestData rdata) {
    getInstance().printLog(cls, message, t, rdata);
  }
