...
<%
  ...
  ArrayList<String> keys = new ArrayList<String>(configs.keySet());
  Collections.sort(keys);
%>
<form class="form-horizontal" action="/_application" method="post" name="form" accept-charset="UTF-8">
  <div class="well">
    <input type="hidden" name="method" value="saveConfiguration"/>
    <legend><%=StringCache.getHtml("configuration")%>
    </legend>
    <div>
      <%for (String key : keys) {%>
      <bandika:controlGroup labelKey="<%=key%>" name="<%=key%>" mandatory="true">
        <input class="input-block-level" type="text" id="<%=key%>" name="<%=key%>" value="<%=FormatHelper.toHtml(configs.get(key))%>" maxlength="255"/>
