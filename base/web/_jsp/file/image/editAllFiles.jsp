...
<%
  ...
  ArrayList<LinkedFileData> files = FileBean.getInstance().getAllPublicFiles(filterData, true);
%>
<form class="form-horizontal" action="/_file" method="post" name="form" accept-charset="UTF-8">
  <div class="well">
    <input type="hidden" name="method" value="openEditFile"/>
    <input type="hidden" name="type" value="<%=filterData.getType()%>"/>
    <legend><%=rdata.getTitle()%>
    </legend>
    <bandika:dataTable id="fileTable" checkId="fid" formName="form" headerKeys="name,preview,usages,dimension" sort="true" paging="true" displayLength="7">
  ...
  <div class="btn-toolbar">
    <button class="btn btn-primary" onclick="return linkTo('/_file?method=openCreateFile');"><%=StringCache.getHtml("new")%>
    </button>
