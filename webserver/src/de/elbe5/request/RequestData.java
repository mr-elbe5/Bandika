...
public class RequestData extends KeyValueMap {
    ...
    public boolean checkFormErrors() {
        ...
        if (formError.isFormIncomplete())
            formError.addFormError(Strings.string("_notComplete",getSessionLocale()));
    ...
    public boolean isPageEditMode() {
        PageData pageData = getCurrentPage();
        return pageData != null && pageData.getViewMode() == ViewMode.EDIT;
    }

    public boolean isPageDetailEditMode() {
        PageData pageData = getCurrentPage();
        return pageData != null && pageData.isDetailEditMode();
    }
