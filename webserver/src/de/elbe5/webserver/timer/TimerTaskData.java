...
public class TimerTaskData extends BaseIdData implements Cloneable {
    ...
    public void setLastExecution(Date lastExecution) {
        if (lastExecution == null) this.lastExecution = TimerBean.getInstance().getServerTime();
        else this.lastExecution = lastExecution;
    }
    ...
    /* cal = lastExecution | now >> lastExpected >> nextExecution  */
        switch (intervalType) {
            ...
            case INTERVAL_TYPE_MONTH: {
                ...
                if (cal.getTime().after(now)) cal.add(Calendar.MONTH, -1);
                if (!(lastExecution.before(cal.getTime()))) cal.add(Calendar.MONTH, 1);
            }
            break;
            case INTERVAL_TYPE_WEEK: {
                ...
                if (cal.getTime().after(now)) cal.add(Calendar.DATE, -7);
                if (!(lastExecution.before(cal.getTime()))) cal.add(Calendar.DATE, 7);
            }
            break;
            case INTERVAL_TYPE_DAY: {
                ...
                cal.set(Calendar.MINUTE, getMinute());
                cal.set(Calendar.SECOND, getSecond());
                if (cal.getTime().after(now)) cal.add(Calendar.DATE, -1);
                if (!(lastExecution.before(cal.getTime()))) cal.add(Calendar.DATE, 1);
            }
            break;
            case INTERVAL_TYPE_HOUR: {
                ...
                cal.set(Calendar.SECOND, getSecond());
                cal.set(Calendar.MILLISECOND, 0);
                if (cal.getTime().after(now)) cal.add(Calendar.HOUR, -1);
                if (!(lastExecution.before(cal.getTime()))) cal.add(Calendar.HOUR, 1);
            }
            break;
            case INTERVAL_TYPE_MINUTE: {
                ...
                cal.set(Calendar.MILLISECOND, 0);
                cal.set(Calendar.SECOND, getSecond());
                if (cal.getTime().after(now)) cal.add(Calendar.MINUTE, -1);
                if (!(lastExecution.before(cal.getTime()))) cal.add(Calendar.MINUTE, 1);
            }
            break;
        }
        return cal.getTime();
    ...
    public boolean execute(Date now) {
        if (task != null) {
            if (task.execute(nextExecution, now)) {
                ...
                setNextExecution(computeNextExecution(now));
                if (noteExecution()) TimerBean.getInstance().updateExcecutionDate(this);
            }
            return true;
        }
        return false;
    ...
    protected void fillProperties(DataProperties properties, Locale locale){
        properties.setHeader(StringUtil.getString("_task", locale));
        properties.addKeyProperty("_id", getId(), locale);
        properties.addKeyProperty("_name", name, locale);
        properties.addKeyProperty("_active",isActive() ? "X" : "-",locale);
        properties.addKeyProperty("_noteExecution",noteExecution() ? "X" : "-",locale);
        properties.addKeyProperty("_intervalType",getIntervalType(),locale);
    }
