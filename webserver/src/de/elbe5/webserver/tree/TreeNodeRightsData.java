...
public class TreeNodeRightsData implements IRights {
    ...
    public void addRight(int id, int right) {
        if (rights.containsKey(id)) rights.put(id, (right | rights.get(id)));
        else rights.put(id, right);
    }
    ...
    public boolean hasRight(int right) {
        for (Integer rgt : rights.values())
            if ((rgt & right) == right) return true;
        return false;
    }
