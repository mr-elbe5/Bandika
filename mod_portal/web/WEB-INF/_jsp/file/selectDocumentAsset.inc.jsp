...
<%
    ...
    List<DocumentData> files = DocumentBean.getInstance().getAllAvailableDocumentsForPage(pageId);
...
<script type="text/javascript">
    function callEditorCallback(imgId) {
        window.opener.CKEDITOR.tools.callFunction(<%=selectData.getCallbackFuncNum()%>, '/document.srv?act=download&fid=' + imgId);
        window.close();
    }
    function callFieldCallback(fileId) {
        opener.setSelLink('/document.srv?act=download&fid=' + fileId);
        window.close();
    }
</script>
<form class="form-horizontal" action="/document.srv" method="post" name="form" accept-charset="UTF-8">
    ...
    <div class="well">
        <bandika:table id="fileTable" formName="form"
                       headerKeys="portal_name,portal_select,portal_preview,portal_exclusive">
            <% for (DocumentData data : files) {%>
            <tr>
                ...
                <td>
                    <div>
                        ...
                        <%}%>
                        <%=StringCache.getHtml("portal_select",locale)%></a>
                    </div>
                </td>
                <td><a href="/document.srv?act=show&fid=<%=data.getId()%>"><%=StringCache.getHtml("portal_preview",locale)%>
                </a></td>
    ...
    <div class="btn-toolbar">
        <button class="btn btn-primary"
                onclick="return linkTo('/document.srv?act=openCreateDocument&pageId=<%=selectData.getPageId()%>');"><%=StringCache.getHtml("webapp_new",locale)%>
        </button>
