...
<%
    ...
    ImageBean bean = ImageBean.getInstance();
%>
<div class="well">
    <bandika:controlText key="reallyDeleteFile"/>
    <% for (Integer id : ids) {
        ImageData data = bean.getImageData(id);
        if (data.hasThumbnail()) {%>
    <div>
        <div>
            <img src="/image.srv?act=showThumbnail&fid=<%=data.getId()%>" border='0' alt="" id="img<%=data.getId()%>"/>
        </div>
        <%}%>
        <div><%=data.getFileName()%>
        </div>
    </div>
    <%}%>
</div>
<div class="btn-toolbar">
    <button class="btn btn-primary"
            onclick="return linkTo('/image.srv?act=deleteImage&fid=<%=StringFormat.getIntString(ids)%>');"><%=StringCache.getHtml("webapp_delete",locale)%>
    </button>
    <button class="btn"
