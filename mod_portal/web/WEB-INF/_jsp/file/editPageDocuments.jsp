...
<%
    ...
    List<DocumentData> files = DocumentBean.getInstance().getCurrentDocumentsOfPage(pageId, false);
%>
<form class="form-horizontal" action="/document.srv" method="post" name="form" accept-charset="UTF-8">
    <div class="well">
        ...
        <bandika:table id="fileTable" checkId="fid" formName="form" headerKeys="portal_name,portal_thumbnail">
            <% for (DocumentData data : files) { %>
            <tr>
                <td><input type="checkbox" name="fid" value="<%=data.getId()%>"/></td>
                <td>
                    <a href="/document.srv?act=openEditDocument&fid=<%=data.getId()%>&pageId=<%=pageId%>"><%=StringFormat.toHtml(data.getFileName())%>
                    </a></td>
                <td>
                    <a href="#">
                        <%=data.getFileName()%></a>
