...
<%
    ...
    List<LayoutTemplateData> templates = ts.getAllLayoutTemplates();
%>
<form class="form-horizontal" action="/template.srv" method="post" name="form" accept-charset="UTF-8">
    ...
    <div class="well">
        ...
        <bandika:table id="templateTable" checkId="tname" formName="form" headerKeys="portal_name">
            <% for (LayoutTemplateData template : templates) { %>
            <tr>
                <td><input type="checkbox" name="tname" value="<%=template.getName()%>"/>
                </td>
                <td>
                    <a href="/template.srv?act=openEditLayoutTemplate&tname=<%=StringFormat.encode(template.getName())%>"><%=StringFormat.toHtml(template.getName())%>
                    </a></td>
            </tr>
            <%
                }
    ...
    <div class="btn-toolbar">
        <button class="btn btn-primary"
                onclick="return linkTo('/template.srv?act=openCreateLayoutTemplate');"><%=StringCache.getHtml("webapp_new",locale)%>
        </button>
        <button class="btn btn-primary"
                onclick="return submitAction('openEditLayoutTemplate');"><%=StringCache.getHtml("webapp_change",locale)%>
        </button>
        <button class="btn btn-primary"
                onclick="return submitAction('openDeleteLayoutTemplates');"><%=StringCache.getHtml("webapp_delete",locale)%>
        </button>
    </div>
</form>
