...
<%
    ...
    List<MasterTemplateData> masterTemplates = TemplateCache.getInstance().getMasterTemplates();
...
<div class="layerContent">
    <table class="table">
        ...
        <tbody>
        <% for (MasterTemplateData tdata : masterTemplates) {%>
        <tr <%=tdata.getName().equals(data.getMasterTemplate()) ? "class=\"info\"" : ""%>>
            <td>
                <a href="/page.srv?act=changeMaster&pageId=<%=pageId%>&master=<%=StringFormat.encode(tdata.getName())%>"><%=StringFormat.toHtml(tdata.getName())%>
                </a></td>
