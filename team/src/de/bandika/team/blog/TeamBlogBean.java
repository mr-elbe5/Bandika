...
public class TeamBlogBean extends Bean {
  ...
  protected void writeEntryData(Connection con, TeamBlogEntryData data) throws SQLException {
    PreparedStatement pst = null;
    try {
      String sql = data.isBeingCreated() ? "insert into t_teamblogentry (change_date,teampart_id,title,author_id,author_name,entry,id) values (?,?,?,?,?,?,?)"
  ...
  public ArrayList<TeamBlogEntryData> getEntryList(int teampartId) {
    ArrayList<TeamBlogEntryData> list = new ArrayList<TeamBlogEntryData>();
    Connection con = null;
    PreparedStatement pst = null;
    try {
      con = getConnection();
  ...
  public void readEntryList(Connection con, PreparedStatement pst, ArrayList<TeamBlogEntryData> list) throws SQLException {
    int i;
    ResultSet rs = pst.executeQuery();
    while (rs.next()) {
      i = 1;
