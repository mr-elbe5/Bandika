...
<table class="table">
  <% for (Integer version : versions) {
    try {
      TeamFileData data = bean.getFileData(id, version);%>
  <tr>
...
<div class="btn-toolbar">
  <button class="btn btn-primary" onclick="return linkTo('/_teamfile?method=deleteHistoryFile&id=<%=cpdata.getPageId()%>&pid=<%=cpdata.getId()%>&fid=<%=id%>&version=<%=StringHelper.getIntString(versions)%>');"><%=StringCache.getHtml("delete", locale)%>
  </button>
  <button class="btn" onclick="return linkTo('/_teamfile?method=openHistory&id=<%=cpdata.getPageId()%>&pid=<%=cpdata.getId()%>&fid=<%=id%>');"><%=StringCache.getHtml("back", locale)%>
  </button>
</div>
