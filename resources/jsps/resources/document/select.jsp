...
<script type="text/javascript">
  ...
  function callFckCallback(docId) {
    window.opener.SetUrl('<%=Statics.DYNAMIC_BASE%>srv25?ctrl=<%=Statics.KEY_DOCUMENT%>&did=' + docId);
    window.close();
  }
  ...
  function DocumentUploadCallback() {
    DocumentUploadWindow.close();
    document.location.href = 'srv25?ctrl=doc&method=reopenSelectDocuments';
  }

  function CallUpload() {
    DocumentUploadWindow = window.open('srv25?ctrl=doc&method=openDocumentUploadSelect', 'DocumentUpload', 'width=490,height=240');
    return false;
  }
  ...
  <table class="adminTable">
    ...
    <tr><% for (int j = 0; j < 3; j++) {
      ...
      <td class="bglightsmall" valign="bottom"><% if (doc != null) {%>
        ...
        <% if (data.isForFck()) {%>
        <a href="#"
           onClick="callFckCallback(<%=doc.getId()%>);return false;"><%=Strings.getHtml("select", sdata.getLocale())%>
        </a>
        <%} else {%>
        <a href="#"
           onClick="callFieldCallback(<%=doc.getId()%>);return false;"><%=Strings.getHtml("select", sdata.getLocale())%>
        </a>
        <%}%>
        <%}%></td>
      <%}%>
