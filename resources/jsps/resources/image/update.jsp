...
<form action="srv25" method="post" name="form" accept-charset="<%=Statics.ISOCODE%>" enctype="multipart/form-data">
  <input type="hidden" name="ctrl" value="<%=Statics.KEY_IMAGE%>"/>
  ...
  <div class="admin">
    <table class="adminTable">
      <tr class="adminLine">
        <td class="adminLeft"><%=Strings.getHtml("image", sdata.getLocale())%>
        </td>
        <td><img src="srv25?ctrl=<%=Statics.KEY_IMAGE%>&method=showThumbnail&iid=<%=img.getId()%>" alt=""></td>
      </tr>
      <tr class="adminLine">
        <td class="adminLeft"><%=Strings.getHtml("imageName", sdata.getLocale())%>
        </td>
        <td><%=Formatter.toHtml(img.getImageName())%>
        </td>
      </tr>
      <tr class="adminLine">
        <td class="adminLeft"><%=Strings.getHtml("imageSize", sdata.getLocale())%>
        </td>
  ...
  <ul class="adminButtonList">
    <li class="adminButton"><a href="#"
                               onClick="submitMethod('updateImage');"><%=Strings.getHtml("save", sdata.getLocale())%>
    </a></li>
    <li class="adminButton"><a
        href="srv25?ctrl=<%=Statics.KEY_IMAGE%>&method=openEditImages"><%=Strings.getHtml("cancel", sdata.getLocale())%>
    </a></li>
  </ul>
</form>
