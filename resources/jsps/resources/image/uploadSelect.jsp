...
<form action="srv25" method="post" name="form" accept-charset="<%=Statics.ISOCODE%>" enctype="multipart/form-data">
  ...
  <input type="hidden" name="method" value="uploadImageSelect"/>
  ...
  <div class="admin">
    <table class="adminTable">
      <tr class="adminLine">
        <td class="adminLeft"><%=Strings.getHtml("file", sdata.getLocale())%>*</td>
        <td><% if (img.getImageName() == null || img.getImageName().length() == 0) {%>
  ...
  <ul class="adminButtonList">
    <li class="adminButton"><a href="#"
                               onClick="submitMethod('uploadImageSelect');"><%=Strings.getHtml("save", sdata.getLocale())%>
    </a></li>
    <li class="adminButton"><a href="javascript:window.close()"><%=Strings.getHtml("cancel", sdata.getLocale())%>
    </a></li>
  </ul>
</form>
