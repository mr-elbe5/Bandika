  ...
  <table class="adminTable">
    ...
    <tr><% for (int j = 0; j < 3; j++) {
      ...
      <td class="bglightsmall" valign="bottom"><% if (img != null) {%>
        ...
        <a href="srv25?ctrl=<%=Statics.KEY_IMAGE%>&method=openImageUpdate&iid=<%=img.getId()%>"><%=Strings.getHtml("change", sdata.getLocale())%>
        </a>
        <a href="srv25?ctrl=<%=Statics.KEY_IMAGE%>&method=openDeleteImage&iid=<%=img.getId()%>"><%=Strings.getHtml("delete", sdata.getLocale())%>
        </a>
        <%}%></td>
      <%}%>
    </tr>
...
<ul class="adminButtonList">
  ...
  <li class="adminButton"><a
      href="srv25?ctrl=<%=Statics.KEY_IMAGE%>&method=openImageUpload"><%=Strings.getHtml("new", sdata.getLocale())%>
  </a></li>
</ul>
