...
<div class="section <%=sectionData.getCss()%>" title="Section <%=sectionData.getName()%>">
    ...
    <div title="<%=title%>" id="part_<%=partData.getId()%>" class="<%=partData.getCss(sectionData.isFlex())%>">
        ...
        <a id="current"></a>
        <form action="/ctrl/templatepage/savePagePart/<%=pageData.getId()%>#current" method="post" id="partform" name="partform" accept-charset="UTF-8">
            ...
            <div class="editpartheader active">
                <a class="btn btn-outline-primary" onclick="evaluateEditFields();return $('#partform').submit();"><%=Strings.html("_ok",locale)%>
                </a>
                <a class="btn btn-outline-secondary" href="/ctrl/templatepage/cancelEditPagePart/<%=pageData.getId()%>"><%=Strings.html("_cancel",locale)%>
                </a>
            </div>
