...
import de.bandika.base.HttpException;
...
public class BaseServlet extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    RequestData reqData = HttpHelper.ensureRequestData(request);
		HttpHelper.ensureSessionData(request);
    if (reqData == null) {
  ...
  protected void prepareMultipartRequestData(HttpServletRequest request, RequestData rdata) throws IOException {
  ...
  protected String readLine(ServletInputStream in, int contentLength, int doneLength) throws IOException {
    ...
    do {
      ...
      if (result != -1)
        sbuf.append(new String(buf, 0, result, HttpHelper.ISOCODE));
  ...
  protected boolean extractNextPart(HttpServletRequest request, ServletInputStream in, String boundary, int contentLength, int doneLength, RequestData rdata) throws IOException {
    String line = readLine(in, contentLength, doneLength);
    if (line == null) {
      return true;
    }
  ...
  protected String[] extractDispositionInfo(String line) throws IOException {
  ...
  protected String extractContentType(String line) throws IOException {
  ...
  protected String extractSingleParameter(ServletInputStream in, String boundary, int contentLength, int doneLength) throws IOException {
    StringBuffer sbuf = new StringBuffer();
    String line;
  ...
  protected byte[] extractSingleData(ServletInputStream in, String boundary) throws IOException {
    ...
    while ((result = in.readLine(bbuf, 0, bbuf.length)) != -1) {
      if (result > 2 && bbuf[0] == '-' && bbuf[1] == '-') {
        line = new String(bbuf, 0, result, HttpHelper.ISOCODE);
        if (line.startsWith(boundary)) break;
      }
      if (rnflag) {
        out.write('\r');
