...
public class PageBean extends Bean {
	...
	protected void readPageFromDb(Connection con, PageData pageData) throws SQLException {
		...
		try {
			pst = con.prepareStatement("select version,parent_id,ranking,name,description,keywords,state,restricted,in_menu,author_id,xml from t_page where id=?");
  ...
  protected void readPageRights(Connection con, PageData data) throws SQLException {
    ...
    PreparedStatement pst3 = null;
		try {
			...
			"insert into t_page (version,parent_id,ranking,name,description,keywords,state,restricted,in_menu,author_id,xml,id) values(?,?,?,?,?,?,?,?,?,?,' ',?)" :
			"update t_page set version=?,parent_id=?,ranking=?,name=?,description=?,keywords=?,state=?,restricted=?,in_menu=?,author_id=?,xml=' ' where id=?");
      ...
      pst1.setBoolean(i++, data.isInMenu());
			pst1.setInt(i++, data.getAuthorId());
