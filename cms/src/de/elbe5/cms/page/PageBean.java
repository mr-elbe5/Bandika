...
public class PageBean extends ResourceBean {
    ...
    public static PageBean getInstance() {
        if (instance == null) instance = new PageBean();
        return instance;
    }
    // *******************************
    ...
    public List<PageData> getAllPages() {
        ...
        try {
            ...
            pst = con.prepareStatement("select t1.id,t1.creation_date,t1.change_date,t1.parent_id,t1.ranking,t1.name," +
                    ...
                    "t2.keywords,t2.published_version,t2.draft_version," +
                    "t3.template " +
                    "from t_treenode t1, t_resource t2, t_page t3 " +
    ...
    public boolean readPage(Connection con, PageData data) throws SQLException {
        ...
        try {
            pst = con.prepareStatement("select template " +
                    "from t_page " +
                    "where id=? ");
    ...
    public boolean createPage(PageData data) {
        ...
        try {
            ...
            if (data.isPublished()) publishPageContent(con, data);
    ...
    protected void writeUsagesByPage(Connection con, PageData data) throws SQLException {
        PreparedStatement pst = null;
        try {
            pst = con.prepareStatement("insert into t_file_page_usage (page_id,page_version,file_id) values(?,?,?)");
            pst.setInt(1, data.getId());
            pst.setInt(2, data.getLoadedVersion());
            HashSet<Integer> list = data.getFileUsage();
            for (int fid : list) {
                pst.setInt(3, fid);
                pst.executeUpdate();
            }
