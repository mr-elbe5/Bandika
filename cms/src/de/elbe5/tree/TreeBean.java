...
public class TreeBean extends DbBean {
    ...
    public boolean readTreeNode(Connection con, TreeNode data) throws SQLException {
        ...
        try {
            pst = con.prepareStatement("SELECT creation_date,change_date,parent_id,ranking,name,display_name,description,owner_id,author_name,visible,anonymous,inherits_rights FROM t_treenode WHERE id=?");
            pst.setInt(1, data.getId());
            try (ResultSet rs = pst.executeQuery()) {
                if (rs.next()) {
                    int i = 1;
    ...
    protected void writeTreeNode(Connection con, TreeNode data) throws SQLException {
        ...
        PreparedStatement pst = null;
        try {
            pst = con.prepareStatement(data.isNew() ? "insert into t_treenode (creation_date,change_date,parent_id,ranking,name,display_name,description,owner_id,author_name,visible,anonymous,inherits_rights,id) values(?,?,?,?,?,?,?,?,?,?,?,?,?)" : "update t_treenode set creation_date=?,change_date=?,parent_id=?,ranking=?,name=?,display_name=?,description=?,owner_id=?,author_name=?,visible=?,anonymous=?,inherits_rights=? where id=?");
            int i = 1;
