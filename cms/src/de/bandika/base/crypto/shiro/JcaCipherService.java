...
public abstract class JcaCipherService implements CipherService {
    ...
    protected static SecureRandom getDefaultSecureRandom() {
        try {
            ...
            Log.warn("The SecureRandom SHA1PRNG algorithm is not available on the current platform.  Using the " + "platform's default SecureRandom algorithm.");
    ...
    private ByteSource encrypt(byte[] plaintext, byte[] key, byte[] iv, boolean prependIv) throws CryptoException {
        ...
        //Log.info("Incoming plaintext of size " + (plaintext != null ? plaintext.length : 0) + ".  Ciphertext " + "byte array is size " + (output != null ? output.length : 0));
