...
import de.bandika.tree.TreeNodeSortData;
...
public enum SiteAction implements ITreeAction {
            ...
            for (SiteData site : sites){
                for (PageData page: site.getPages()){
                    if (page.getDraftVersion()>page.getPublishedVersion()){
                        ...
                        draft.setAuthorName(SessionReader.getUserName(request));
                        draft.prepareSave();
    ...
    inheritAll {
        ...
        public boolean execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
            ...
            data.getAllSites(sites);
            for (SiteData site : sites){
                site.setAuthorName(SessionReader.getUserName(request));
                site.setAnonymous(anonymous);
                site.setInheritsRights(true);
                site.prepareSave();
                SiteBean.getInstance().saveSiteSettings(site);
                for (PageData page: site.getPages()){
                    page.setAuthorName(SessionReader.getUserName(request));
                ...
                for (FileData file: site.getFiles()){
                    file.setAuthorName(SessionReader.getUserName(request));
    ...
    /**
     * delete the site including children from database
     */
    delete {
        @Override
        public boolean execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    /**
     * open dialog for creating sub structure from xml
     */
    openImportFromXml {
        @Override
        public boolean execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    protected boolean showSiteDetails(HttpServletRequest request, HttpServletResponse response) {return sendForwardResponse(request, response, "/WEB-INF/_jsp/site/siteDetails.ajax.jsp");}
