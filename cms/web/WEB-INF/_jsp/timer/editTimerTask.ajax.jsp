...
<%Locale locale = SessionHelper.getSessionLocale(request);
    TimerTaskData data = (TimerTaskData) SessionHelper.getSessionObject(request, "timerTaskData");
    if ((data == null)) throw new AssertionError();%>
<jsp:include page = "/WEB-INF/_jsp/_masterinclude/error.inc.jsp"/>
<form action = "/timer.srv" method = "post" id = "taskform" name = "taskform" accept-charset = "UTF-8">
    ...
    <fieldset>
        <table class = "form">
            ...
            <tr>
                ...
                <td>
                    <div>
                        <input type = "radio" name = "intervalType"
                                value = "<%=TimerTaskData.INTERVAL_TYPE_CONTINOUS%>"<%=data.getIntervalType() == TimerTaskData.INTERVAL_TYPE_CONTINOUS ? "checked=\"checked\"" : ""%> />&nbsp;<%=StringUtil.getHtml("_continous", locale)%>
                    </div>
                    <div><input type = "radio" name = "intervalType"
                            value = "<%=TimerTaskData.INTERVAL_TYPE_MONTH%>"<%=data.getIntervalType() == TimerTaskData.INTERVAL_TYPE_MONTH ? "checked=\"checked\"" : ""%> />&nbsp;<%=StringUtil.getHtml("_monthly", locale)%>
                    </div>
                    <div><input type = "radio" name = "intervalType"
                            value = "<%=TimerTaskData.INTERVAL_TYPE_WEEK%>"<%=data.getIntervalType() == TimerTaskData.INTERVAL_TYPE_WEEK ? "checked=\"checked\"" : ""%> />&nbsp;<%=StringUtil.getHtml("_weekly", locale)%>
                    </div>
                    <div><input type = "radio" name = "intervalType"
                            value = "<%=TimerTaskData.INTERVAL_TYPE_DAY%>"<%=data.getIntervalType() == TimerTaskData.INTERVAL_TYPE_DAY ? "checked=\"checked\"" : ""%> />&nbsp;<%=StringUtil.getHtml("_daily", locale)%>
                    </div>
                    <div><input type = "radio" name = "intervalType"
                            value = "<%=TimerTaskData.INTERVAL_TYPE_HOUR%>"<%=data.getIntervalType() == TimerTaskData.INTERVAL_TYPE_HOUR ? "checked=\"checked\"" : ""%> />&nbsp;<%=StringUtil.getHtml("_everyHour", locale)%>
                    </div>
                    <div><input type = "radio" name = "intervalType"
                            value = "<%=TimerTaskData.INTERVAL_TYPE_MINUTE%>"<%=data.getIntervalType() == TimerTaskData.INTERVAL_TYPE_MINUTE ? "checked=\"checked\"" : ""%> />&nbsp;<%=StringUtil.getHtml("_everyMinute", locale)%>
                    </div>
                </td>
            </tr>
            <tr>
            ...
            <tr>
                <td>
                    <label for = "active"><%=StringUtil.getHtml("_noteExecution", locale)%>
                    </label></td>
                <td>
