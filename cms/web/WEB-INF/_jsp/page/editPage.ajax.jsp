    ...
    <div class="modal-content">
        ...
        <cms:form url="<%=url%>" name="pageform" ajax="true">
            <div class="modal-body">
                ...
                <cms:line label="<%=Strings._changeDate.toString()%>"><%=StringUtil.toHtmlDateTime(pageData.getChangeDate(), locale)%></cms:line>
                ...
                <cms:line label="<%=label%>" padded="true">
                    <cms:radio name="<%=name%>" value="" checked="<%=!pageData.hasAnyGroupRight(group.getId())%>"><%=Strings._rightnone.html(locale)%></cms:radio><br/>
                    <cms:radio name="<%=name%>" value="<%=Right.READ.name()%>" checked="<%=pageData.isGroupRight(group.getId(), Right.READ)%>"><%=Strings._rightread.html(locale)%></cms:radio><br/>
                    <cms:radio name="<%=name%>" value="<%=Right.EDIT.name()%>" checked="<%=pageData.isGroupRight(group.getId(), Right.EDIT)%>"><%=Strings._rightedit.html(locale)%></cms:radio><br/>
                    <cms:radio name="<%=name%>" value="<%=Right.APPROVE.name()%>" checked="<%=pageData.isGroupRight(group.getId(), Right.APPROVE)%>"><%=Strings._rightapprove.html(locale)%></cms:radio><br/>
                </cms:line>
                <%}}%>
                <% if (!pageData.isNew()){%>
                <h3><%=Strings._subpages.html(locale)%></h3>
                    <cms:line label="<%=Strings._name.toString()%>"><%=Strings._position.html(locale)%></cms:line>
                    <%int idx = 0;
