...
<li<%=groupId != 0 ? " class=\"open\"" : ""%>>
    ...
    <div class="contextMenu">
        <div class="icn inew" onclick="return openLayerDialog('<%=StringUtil.getHtml("_newGroup",locale)%>', '/group.ajx?act=openCreateGroup')"><%=StringUtil.getHtml("_new", locale)%>
        </div>
    </div>
    <ul>
        <%if (groups != null) {
        ...
        <li>
            ...
            <div class="contextMenu">
                <div class="icn iedit" onclick="return openLayerDialog('<%=StringUtil.getHtml("_editGroup",locale)%>', '/group.ajx?act=openEditGroup&groupId=<%=group.getId()%>');"><%=StringUtil.getHtml("_edit", locale)%>
                </div>
                <% if (group.getId() != GroupData.ID_ALL) {%>
                <div class="icn iadd" onclick="return openLayerDialog('<%=StringUtil.getHtml("_addUser",locale)%>', '/group.ajx?act=openAddGroupUser&groupId=<%=group.getId()%>');"><%=StringUtil.getHtml("_addUser", locale)%>
                </div>
                <div class="icn iremove" onclick="return openLayerDialog('<%=StringUtil.getHtml("_removeUsers",locale)%>', '/group.ajx?act=openRemoveGroupUsers&groupId=<%=group.getId()%>');"><%=StringUtil.getHtml("_removeUsers", locale)%>
                </div>
