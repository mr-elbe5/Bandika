...
function openSetImage(ident) {
	imageIdent = ident;
	imgSelectPopup = window.open("/_image?method=openImageSelector&popup=1", "ImageSelector", "width=688,height=500,resizable=yes,scrollbars=yes");
	return false;
}
...
function setImage(imgId, imgWidth, imgHeight, altText) {
	...
	if (img) {
		img.src = "/_image?method=show&iid=" + imgId;
		img.alt = altText;
		if (imgWidth > 0)
			img.width = imgWidth;
		if (imgHeight > 0)
...
function openSetDocument(ident) {
	docIdent = ident;
	docSelectPopup = window.open("/_doc?method=openDocumentSelector&popup=1", "DocumentSelector", "width=688,height=500,resizable=yes,scrollbars=yes");
	return false;
}
...
function setDocument(docId) {
  ...
  var doc = document.getElementById(docIdent);
	if (doc) {
		doc.src = "/_doc?method=show&did=" + docId;
	}
}
